...
```bash
LUCID_DATA_ROOT=/sdf/data/neutrino/omara/wand_sk_like/config_000001 pytest
...
The canonical real LUCiD-format dataset at SLAC is the WAND
SK-like sample, exposed as 18 configs under:

```
/sdf/data/neutrino/omara/wand_sk_like/
  config_000001/  single mu-
  config_000002/  single pi+
  ...
  config_000013/  GENIE numu          (101k events, 77 shards)
  config_000018/  pile-up 2 × bomb    (50k events, 67 shards)
```

The directories are symlinked into the v3 layout from the upstream
`/sdf/data/neutrino/cjesus/DORAEMON/WAND/SK_like/` (which still uses
the pre-rename `seg/inst` directory names) — no extra storage cost.
...
| Single large config (e.g. `config_000003`, 146 shards) | ~40 s |
