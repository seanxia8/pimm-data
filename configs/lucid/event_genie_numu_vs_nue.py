"""Data-loader recipe — LUCiD event classification: GENIE numu vs nue (flavor).

CAMPAIGN.md: LUCiD | Event class/regression | realistic neutrino flavor ID on
GENIE CC interactions (0.1-2 GeV water). MultiModalEventDataset over the two
GENIE WAND configs. Data-loading half only; model is a placeholder.
"""
import os
...
_sources = [
    dict(name="config_000013", label=0, config_id=0),   # GENIE numu, water, 0.1-2 GeV
    dict(name="config_000014", label=1, config_id=1),   # GENIE nue,  water, 0.1-2 GeV
]
...
data = dict(
    num_classes=2, names=["numu", "nue"], ignore_index=-1,
