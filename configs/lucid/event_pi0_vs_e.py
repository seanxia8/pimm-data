"""Data-loader recipe — LUCiD event classification: pi0 vs e- (ring counting).

CAMPAIGN.md: LUCiD | Event class/regression | the classic NC-pi0 background:
pi0 -> 2 gamma -> two overlapping rings, easily faked as a single e- ring.
MultiModalEventDataset over the e- and pi0 single-particle WAND configs.
...
_sources = [
    dict(name="config_000003", label=0, config_id=0),   # e-  (single ring)
    dict(name="config_000005", label=1, config_id=1),   # pi0 (two rings)
]
...
data = dict(
    num_classes=2, names=["e-", "pi0"], ignore_index=-1,
