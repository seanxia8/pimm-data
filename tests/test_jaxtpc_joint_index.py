...
sets diverge — ``min_deposits>0`` masks edep only, or a production gap is
...
every modality for every idx (A5), plus A3 (min_deposits without edep raises;
...
def test_gap_in_one_modality_realigns(tmp_path):
    ...
    with h5py.File(os.path.join(root, 'sensor', 'sim_sensor_0000.h5'), 'r+') as f:
        del f['event_001']                       # present in edep/hits/labl
...
def test_raw_readers_desync_without_joint_index(tmp_path):
    ...
    This is what the dataset did pre-Phase-A. edep keeps all 3 events, sensor
    ...
    edep = JAXTPCEdepReader(data_root=os.path.join(root, 'edep'), split='',
                            dataset_name='sim')
    sensor = JAXTPCSensorReader(data_root=os.path.join(root, 'sensor'),
                                split='', dataset_name='sim')
    assert edep.indices[0].tolist() == [0, 1, 2]
    assert sensor.indices[0].tolist() == [0, 2]    # gap → different mapping
    edep.h5py_worker_init(); sensor.h5py_worker_init()
    # At idx 1 the two readers point at DIFFERENT physics events — the desync
    # the joint index removes.
    assert edep._locate_event(1)[1] != sensor._locate_event(1)[1]


# ---------------------------------------------------------------------------
# §4 #1 — min_deposits masks edep only.
# ---------------------------------------------------------------------------

def test_min_deposits_desync_realigns(tmp_path):
    """min_deposits drops a low-deposit edep event; joint drops it everywhere."""
    root = make_jaxtpc_sample(str(tmp_path), n_events=3, n_volumes=2)
    # Zero event_001's deposits in edep so min_deposits filters it from edep
...
# ---------------------------------------------------------------------------
# A3 — min_deposits without edep raises; volume-aware counting.
# ---------------------------------------------------------------------------

def test_min_deposits_without_edep_raises(tmp_path):
